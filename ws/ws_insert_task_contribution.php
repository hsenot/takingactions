...
try {
    $p_task_id = $_REQUEST['task_id'];
    $p_url = $_REQUEST['photo_url'];    
}
catch (Exception $e) {
...
try {
    ...
    $sql = "INSERT INTO task_contribution (task_id,photo_link) VALUES ('".$p_task_id."','".$p_url."');";
    $sql = sanitizeSQL($sql);
    //echo $sql;
    ...
    $sql = "SELECT last_value from task_contribution_id_seq;";
    $sql = sanitizeSQL($sql);
    //echo $sql;
    $recordSet = $pgconn->prepare($sql);
    $recordSet->execute();
